...
export default function ManageBranch({
  ...
  const handleChange = (name: string, value: string) => {
    // Enforce max lengths and numeric-only for specific fields
    ...
    if (fieldLimits[name] && value.length > fieldLimits[name]) return;
  ...
  const inputClass = (field: string) =>
    `w-full px-3 py-2 border rounded-lg ${errors[field] ? 'border-red-500' : 'border-gray-300'}`;
    ...
    <div className={`bg-white rounded-lg shadow-lg p-6 ${isDisabled ? 'opacity-50' : ''}`}>
      ...
      {isDisabled && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-6">
          <p className="text-amber-800">You need to create at least one bank before managing branches.</p>
        </div>
      )}
